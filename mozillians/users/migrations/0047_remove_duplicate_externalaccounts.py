...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        unique_fields = ['identifier', 'type', 'user']

        # List groups of duplicate account objects and lowest id in the group.
        duplicates = (orm.ExternalAccount.objects.values(*unique_fields)
                                         .annotate(min=models.Min('id'),
                                                   count=models.Count('id'))
                                         .filter(count__gt=1).order_by())

        # Delete all of the objects in each duplicate group, except the lowest id.
        for duplicate in duplicates:
            (orm.ExternalAccount.objects.filter(**dict((x, duplicate[x]) for x in unique_fields))
                    .exclude(id=duplicate['min'])
                    .delete())

    def backwards(self, orm):
        "Can't go backwards from deleting duplicates."
