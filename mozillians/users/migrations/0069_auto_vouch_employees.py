...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for auto_user in orm['auth.User'].objects.filter(AV_QUERY):
            ...
            if vouch and not vouch[0].voucher:
                ...
                vouch = vouch[0]
                vouch.description = 'An automatic vouch for being a Mozilla employee.'
                vouch.autovouch = True
                vouch.date = vouch.date or now
                vouch.save()
            else:
    ...
    def backwards(self, orm):
        ...
        users = orm['auth.User'].objects.filter(AV_QUERY)
        vouches = orm['users.Vouch'].objects.filter(
            vouchee__user__in=users,
            autovouch=True,
            description='An automatic vouch for being a Mozilla employee.')
