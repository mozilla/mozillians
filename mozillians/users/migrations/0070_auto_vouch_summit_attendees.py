...
SUMMIT_LOCATIONS = ['summit2013-brussels', 'summit2013-santa-clara', 'summit2013-toronto']
SUMMIT_GROUPS_QUERY = reduce(
    or_, [Q(name__iexact='{0}'.format(e)) for e in SUMMIT_LOCATIONS])
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        attendees = orm['users.UserProfile'].objects.filter(groups__in=groups).distinct()
        ...
        for attendee in attendees:
            for i in range(1, 4):
                orm.Vouch.objects.create(
                    ...
                    autovouch=True,
                    date=now,
                    description='An automatic vouch for attending Summit 2013 Day {0}'.format(i)
                )

    def backwards(self, orm):
        groups = orm['groups.Group'].objects.filter(SUMMIT_GROUPS_QUERY)
        attendees = orm['users.UserProfile'].objects.filter(groups__in=groups)
        vouches = orm['users.Vouch'].objects.filter(
            vouchee__user__userprofile__in=attendees,
            autovouch=True,
            description__icontains='An automatic vouch for attending Summit 2013 Day')
