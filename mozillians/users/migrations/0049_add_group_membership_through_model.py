...
class Migration(SchemaMigration):
    # Don't run this until we've run the groups migration that copies
    # the data to the new through table
    depends_on = (
        ('groups', '0016_migrate_group_membership'),
    )

    def forwards(self, orm):
        # Removing M2M table for field groups on 'UserProfile'
        db.delete_table(db.shorten_name('profile_groups'))


    def backwards(self, orm):
        # Adding M2M table for field groups on 'UserProfile'
        m2m_table_name = db.shorten_name('profile_groups')
